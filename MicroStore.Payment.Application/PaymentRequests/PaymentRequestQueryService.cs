...
namespace MicroStore.Payment.Application.PaymentRequests
{
    public class PaymentRequestQueryService : PaymentApplicationService, IPaymentRequestQueryService
    {
        ...
        public async Task<UnitResultV2<PaymentRequestDto>> GetAsync(string paymentId, CancellationToken cancellationToken = default)
        {
        ...
        public async Task<UnitResultV2<PaymentRequestDto>> GetByOrderIdAsync(string orderId, CancellationToken cancellationToken = default)
        {
            var query = _paymentDbContext.PaymentRequests
              .AsNoTracking()
              .ProjectTo<PaymentRequestDto>(MapperAccessor.Mapper.ConfigurationProvider);
            ...
            if (result == null)
            {
                return UnitResultV2.Failure<PaymentRequestDto>(ErrorInfo.NotFound($"Payment request with order id : {orderId} is not exist"));
            }
        ...
        public async Task<UnitResultV2<PaymentRequestDto>> GetByOrderNumberAsync(string orderNumber, CancellationToken cancellationToken = default)
        {
            var query = _paymentDbContext.PaymentRequests
            .AsNoTracking()
            .ProjectTo<PaymentRequestDto>(MapperAccessor.Mapper.ConfigurationProvider);
            ...
            if (result == null)
            {
                return UnitResultV2.Failure<PaymentRequestDto>(ErrorInfo.NotFound($"Payment request with order number : {orderNumber} is not exist"));
            }
        ...
        public async Task<UnitResultV2<PagedResult<PaymentRequestListDto>>> ListPaymentAsync(PagingAndSortingQueryParams queryParams, string? userId = null, CancellationToken cancellationToken = default)
        {
            var query = _paymentDbContext.PaymentRequests
              .AsNoTracking()
