...
namespace MicroStore.ShoppingGateway.ClinetSdk.Services.Orders
{
    public class ProductAnalysisService
    {
        ...
        public async Task<PagedList<ProductSalesReport>> GetProductSalesReport( string productId, ProductSalesReportRequestOptions options, CancellationToken cancellationToken = default)
        {
            string path = string.Format("{0}/{1}/{2}", BaseUrl, productId, "units-summary");

            return await _microStoreClinet.MakeRequest<PagedList<ProductSalesReport>>(path, HttpMethod.Get, options, cancellationToken);
        }
    }
}
