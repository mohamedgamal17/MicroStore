    ...
    public class OrderAggregateService : IListableWithPaging<OrderAggregate, OrderListRequestOptions>
    {
        ...
        private async Task<OrderAggregate> PreapreOrderAggregate(Order order, CancellationToken cancellationToken = default)
        {
            ...
            var aggregate = new OrderAggregate
            {
                ...
                Payment = !string.IsNullOrEmpty(order.PaymentId) ? PreaprePaymentPaymentRequestAggregate(paymentRequest.Result, userProfile.Result) : null,
                ShipmentId = order.ShipmentId,
                Shipment = !string.IsNullOrEmpty(order.ShipmentId)? PreapreShipmentAggregate(shipment.Result, shippingAddress.Result, userProfile.Result) : null,
                Items = order.Items,
