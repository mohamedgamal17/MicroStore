...
{
    public class BasketTokenExchangeDelegatingHandler : BaseTokenExchangeDelegatingHandler
    {
        const string CacheKey = "mvcgatewaytodownstreamtokenexchangeclient_basket";
        public BasketTokenExchangeDelegatingHandler(IHttpClientFactory httpClinetFactory, IClientAccessTokenCache clientAccessTokenCache,       IHttpContextAccessor httpContextAccessor, IOptions<IdentityProviderOptions> identityProviderOptions, IOptions<GatewayClientOptions> gatewayClientOptions) : base(httpClinetFactory, clientAccessTokenCache, httpContextAccessor, identityProviderOptions, gatewayClientOptions)
        {
        }
        protected override List<string> RequiredScopes => BasketScope.List();
