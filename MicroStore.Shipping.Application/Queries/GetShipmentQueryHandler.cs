...
namespace MicroStore.Shipping.Application.Queries
{
    public class GetShipmentQueryHandler : QueryHandler<GetShipmentQuery>
    {
        ...
        public GetShipmentQueryHandler(IShippingDbContext shippingDbContext)
        {
        ...
        public override async Task<ResponseResult> Handle(GetShipmentQuery request, CancellationToken cancellationToken)
        {
            ...
            var result = await query.SingleOrDefaultAsync(x => x.ShipmentId == request.ShipmentId, cancellationToken);

            if (result == null)
            {
                return Failure(HttpStatusCode.NotFound, $"shipment with id {request.ShipmentId} is not exist");
            }
