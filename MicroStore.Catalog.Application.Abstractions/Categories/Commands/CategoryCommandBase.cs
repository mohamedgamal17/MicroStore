...
namespace MicroStore.Catalog.Application.Abstractions.Categories.Commands
{
    ...
    internal abstract class CategoryCommandValidatorBase<TCommand> : AbstractValidator<TCommand>
     where TCommand : CategoryCommandBase
    {
        public CategoryCommandValidatorBase()
        {
            RuleFor(x => x.Name)
                .NotEmpty()
